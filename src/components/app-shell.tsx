...
export function AppShell({ children, unreadNotificationsCount = 0 }: AppShellProps) {
  const pathname = usePathname();

  return (
    <main className="mx-auto min-h-screen w-full max-w-5xl px-4 pb-28 pt-4 sm:px-5 sm:pb-10 sm:pt-6">
      <header className="sticky top-0 z-30 -mx-4 mb-5 border-b border-slate-200/80 bg-slate-50/90 px-4 py-3 backdrop-blur dark:border-slate-800/80 dark:bg-slate-950/90 sm:static sm:mx-0 sm:mb-8 sm:rounded-[2rem] sm:border sm:bg-white/80 sm:px-5 dark:sm:bg-slate-950/80">
        <div className="flex items-center justify-between gap-3">
          ...
          <nav className="hidden items-center gap-2 text-sm font-semibold sm:flex">
            {appNavItems.filter((item) => item.href !== "/notifications").map((item) => {
          ...
          {appNavItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={cn(
                  "relative flex flex-col items-center justify-center gap-1 rounded-2xl px-2 py-2 text-[0.68rem] font-bold text-slate-500 transition active:scale-95 dark:text-slate-400",
                  isActive && "bg-indigo-50 text-indigo-700 dark:bg-indigo-950/70 dark:text-indigo-200",
                )}
                href={item.href}
                key={item.href}
              >
                ...
                {item.href === "/notifications" && unreadNotificationsCount > 0 ? <Badge count={unreadNotificationsCount} compact /> : null}
              </Link>
            );
          })}
        </div>
...
function Badge({ count, compact = false }: { count: number; compact?: boolean }) {

  ...
  const [
    ...
    prisma.friendship.findMany({
      where: {
        ...
        status: "PENDING",
        requester: noBlockBetweenWhere(session.user.id),
      },
      orderBy: { createdAt: "desc" },
      select: {
        id: true,
        requester: { select: { name: true, username: true, image: true } },
      },
    }),
    prisma.friendship.findMany({
      where: {
        ...
        status: "PENDING",
        receiver: noBlockBetweenWhere(session.user.id),
      },
      orderBy: { createdAt: "desc" },
      select: {
        id: true,
        receiver: { select: { name: true, username: true, image: true } },
      },
    }),
    prisma.friendship.findMany({
      where: {
      ...
      <section className="space-y-6">
          ...
          <div className="px-6 pb-6">
            <div className="-mt-12 flex items-end justify-between gap-4">
              ...
              <Link
                ...
                className="inline-flex size-11 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 transition hover:text-indigo-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200"
                href="/settings"
              >
            ...
            <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                <Link
                  className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-black text-slate-700 transition hover:border-indigo-200 hover:text-indigo-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 dark:hover:text-indigo-300"
                  href="/stats"
                >
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <section className="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-950">
            ...
            {incomingRequests.length === 0 ? (
              <EmptyState>No tienes solicitudes pendientes.</EmptyState>
            ) : null}
            {incomingRequests.map((request) => (
              <article
                className="space-y-3 rounded-3xl border border-slate-200 p-4 dark:border-slate-800"
                key={request.id}
              >
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  ...
                  <form action={rejectFriendRequestAction}>
                    <input
                      ...
                      value={request.id}
                    />
                    <SubmitButton pendingText="Rechazando...">
                      Rechazar
                    </SubmitButton>
                  </form>
                </div>
        ...
        <section className="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center justify-between gap-4">
            ...
            <Link
              className="text-sm font-black text-indigo-600 dark:text-indigo-300"
              href="/friends"
            >
              Ver/buscar
            </Link>

...
function UserIdentity({ user }: { user: SafeUser }) {
  return (
    <div className="flex items-center gap-3">
      <UserAvatar className="rounded-2xl" user={user} />
      <div>
...
export default async function FriendsPage({ searchParams }: FriendsPageProps) {
  ...
  const [
    ...
    prisma.friendship.findMany({
      where: {
        ...
        status: "PENDING",
        requester: noBlockBetweenWhere(currentUserId),
      },
    ...
    prisma.friendship.findMany({
      where: {
        ...
        status: "PENDING",
        receiver: noBlockBetweenWhere(currentUserId),
      },
      orderBy: { createdAt: "desc" },
      select: {
        id: true,
    ...
    searchTerm.length >= 2
      ? prisma.user.findMany({
          where: {
            ...
            AND: [noBlockBetweenWhere(currentUserId)],
            OR: [
  ...
  const pairKeys = searchResults.map((user) =>
    friendshipPairKey(currentUserId, user.id),
  );
  ...
  const friendshipByPairKey = new Map(
    existingFriendships.map((friendship) => [friendship.pairKey, friendship]),
  );
      ...
      <section className="space-y-6">
        <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-950">
          ...
          <p className="mt-3 max-w-2xl leading-7 text-slate-600 dark:text-slate-300">
            Busca usuarios, envía solicitudes y controla quién puede ver tus
            dailies con visibilidad Amigos.
          </p>
        </div>
        ...
        <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-950">
          ...
          <div className="mt-5 space-y-3">
            ...
            {searchTerm.length >= 2 && searchResults.length === 0 ? (
              <EmptyState>
                No encontramos usuarios con ese nombre o username.
              </EmptyState>
            ) : null}
              ...
              return (
                ...
                >
                  ...
                  {friendship?.status === "ACCEPTED" ? (
                    <span className="rounded-full bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-200">
                      Ya sois amigos
                    </span>
                  ) : friendship?.status === "PENDING" ? (
                    <span className="rounded-full bg-amber-50 px-3 py-2 text-sm font-bold text-amber-700 dark:bg-amber-950/40 dark:text-amber-200">
                      Solicitud pendiente
                    </span>
                  ) : (
                    <form
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <section className="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-950">
            <h2 className="flex items-center gap-2 text-xl font-black text-slate-950 dark:text-white">
              <UserPlus className="size-5" /> Solicitudes recibidas
            </h2>
            ...
            {incomingRequests.map((request) => (
              <article
                key={request.id}
                className="space-y-4 rounded-3xl border border-slate-200 p-4 dark:border-slate-800"
              >
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  <form action={rejectFriendRequestAction} className="flex-1">
                    ...
                    />
                    <button
                      className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-rose-200 px-4 py-3 text-sm font-bold text-rose-700 transition hover:bg-rose-50 dark:border-rose-900/70 dark:text-rose-200 dark:hover:bg-rose-950/40"
                      type="submit"
                    >
          ...
          <section className="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-950">
            <h2 className="flex items-center gap-2 text-xl font-black text-slate-950 dark:text-white">
              <Send className="size-5" /> Solicitudes enviadas
            </h2>
        ...
        <section className="space-y-4 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-xl shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-950">
          <h2 className="flex items-center gap-2 text-xl font-black text-slate-950 dark:text-white">
            <Users className="size-5" /> Lista de amigos
          </h2>
          {friends.length === 0 ? (
            <EmptyState>Todavía no tienes amigos aceptados.</EmptyState>
          ) : null}

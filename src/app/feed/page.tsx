...
import { FeedUserSearch } from "@/components/feed-user-search";
import {
  LockedPostCard,
  PostCard,
  type LockedPostCardPost,
  type PostCardPost,
...
import { visibleAuthorNotBlockedWhere } from "@/lib/blocks";
          ...
          AND: [
            {
              OR: [
                ...
                {
                  visibility: "FRIENDS",
                  user: friendsOnlyAuthorWhere(session.user.id),
                },
              ],
            },
            visibleAuthorNotBlockedWhere(session.user.id),
          ],
        },
        ...
        <section className="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-xl shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-start justify-between gap-4 p-5">
              ...
              {todaysOwnPost ? (
                <>
                  ...
                  <p className="mt-1 text-sm font-semibold text-slate-500 dark:text-slate-400">
                    {formatLongDate(startOfToday)} · Nota{" "}
                    {formatRating(todaysOwnPost.rating)}/10
                  </p>
                </>
              ) : (
                <>
                  <h1 className="mt-3 text-2xl font-black tracking-tight text-slate-950 dark:text-white">
                    Publica tu daily para desbloquear el feed
                  </h1>
                  <p className="mt-2 leading-7 text-slate-600 dark:text-slate-300">
                    Comparte tu nota de hoy y accede a las publicaciones
                    visibles de amigos y comunidad.
                  </p>
                </>
              )}
            </div>
            <Link
              ...
              href={
                todaysOwnPost ? `/posts/${todaysOwnPost.id}/edit` : "/day/new"
              }
            >
        ...
        {!isFeedUnlocked ? (
          <div className="rounded-[1.75rem] border border-amber-200 bg-amber-50 p-5 shadow-xl shadow-amber-950/5 dark:border-amber-900/60 dark:bg-amber-950/30">
            ...
            <p className="mt-3 leading-7 text-slate-700 dark:text-slate-200">
              Mientras tanto solo puedes ver quién ha publicado hoy, sin revelar
              nota, título, descripción ni foto.
            </p>
          </div>
        ) : null}
        ...
        (!isFeedUnlocked && lockedPosts.length === 0) ? (
          <div className="rounded-[1.75rem] border border-dashed border-slate-300 bg-white p-8 text-center dark:border-slate-700 dark:bg-slate-950">
            ...
            <h2 className="mt-4 text-2xl font-black text-slate-950 dark:text-white">
              {isFeedUnlocked
                ? "Todavía no hay dailies visibles hoy"
                : "Todavía no hay personas visibles en el feed"}
            </h2>
          </div>
        ) : null}
      </section>

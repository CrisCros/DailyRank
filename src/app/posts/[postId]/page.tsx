...
import { Heart, MessageCircle, MoreHorizontal, Pencil, Reply, Trash2 } from "lucide-react";
...
export default async function PostDetailPage({
  ...
  const isLikedByCurrentUser = post.likes.some((like) => like.userId === session.user.id);
          ...
          <div className="space-y-5 p-5">
            ...
            <div className="flex items-center gap-2 border-y border-slate-100 py-2 dark:border-slate-800">
              ...
              <details className="relative">
                <summary className="inline-flex cursor-pointer list-none items-center justify-center gap-2 rounded-full px-3 py-2 text-sm font-black text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-900">
                  <Heart className="size-5" /> Ver likes
                </summary>
                <div className="absolute left-0 z-20 mt-2 w-72 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-xl dark:border-slate-800 dark:bg-slate-950 sm:w-80">
                  <div className="border-b border-slate-100 px-4 py-3 dark:border-slate-800">
                    <p className="text-sm font-black text-slate-950 dark:text-white">Usuarios que dieron like</p>
                  </div>
                  <div className="max-h-80 overflow-y-auto p-2">
                    {post.likes.length === 0 ? (
                      <p className="rounded-2xl border border-dashed border-slate-300 p-4 text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
                        Todavía no hay likes visibles.
                      </p>
                    ) : (
                      post.likes.map((like) => (
                        <Link
                          className="flex items-center gap-3 rounded-2xl p-3 transition hover:bg-slate-100 dark:hover:bg-slate-900"
            ...
            <section className="space-y-4" id="comments">
              ...
              <div className="space-y-3">
                ...
                {post.comments.map((comment) => {
                  const replyAction = createReplyAction.bind(null, post.id, comment.id);
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex min-w-0 items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">
                              @{comment.user.username} · {formatDateTime(comment.createdAt)}
                            </p>
                          </div>
                        </div>
                        {canDeleteComment ? (
                      ...
                      <details className="mt-3 rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950/60">
                        <summary className="inline-flex cursor-pointer list-none items-center gap-2 text-sm font-black text-indigo-600 dark:text-indigo-300">
                          <Reply className="size-4" /> Responder
                        </summary>
                        <form action={replyAction} className="mt-3 space-y-3">
                          <textarea
                            className="min-h-20 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100 dark:border-slate-800 dark:bg-slate-900 dark:text-white dark:focus:ring-indigo-950"
                            maxLength={500}
                            name="content"
                          ...
                          <div className="sm:max-w-44">
                            <SubmitButton pendingText="Respondiendo...">
                              Publicar respuesta
                            </SubmitButton>
                          </div>
                        </form>
                      ...
                      {comment.replies.length > 0 ? (
                        <div className="mt-4 space-y-3 border-l-2 border-indigo-100 pl-3 dark:border-indigo-950">
                          {comment.replies.map((reply) => {
                            const canDeleteReply = reply.userId === session.user.id || isOwner;

                            return (
                              <article className="rounded-2xl bg-white p-3 dark:bg-slate-950/60" key={reply.id}>
                                <div className="flex items-start justify-between gap-3">
                                  <div className="flex min-w-0 items-center gap-3">
                                    ...
                                    <div className="min-w-0">
                                      ...
                                      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">
                                        @{reply.user.username} · {formatDateTime(reply.createdAt)}
                                      </p>
                                    </div>
                                  </div>
                                  {canDeleteReply ? (
                                    <form action={deleteCommentAction.bind(null, reply.id)}>
                                      <button
                                        className="rounded-full p-2 text-rose-600 hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-950/40"
                                        type="submit"
